...
def main():
    ...
    if len(checksum_files) == 4:
        ...
        payload_to_sign = f"""\
local_hashes="{local_hashes}" && remote_hashes=$(curl https://pypi.org/pypi/restmesh/json | jq '.urls[] | .digests | to_entries[] | select(.key != "blake2b_256") | .value' | tr -d '"' | sort | tr -d "\\n") && [ ${{local_hashes}} = ${{remote_hashes}} ] && export PIP_ONLY_BINARY=:all: && pipx install {pkg_name}=={version}"""
